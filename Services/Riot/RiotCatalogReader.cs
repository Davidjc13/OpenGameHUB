...
namespace OpenGameHUB.Services.Riot;
...
internal static class RiotCatalogReader
{
    ...
    private static IEnumerable<string> ReadInstallsJsonPaths()
    {
        ...
        try
        {
            ...
            if (!document.RootElement.TryGetProperty("associated_client", out var associatedClient)
            ...
            {
                var candidate = property.Name.Trim().Trim('"');
    ...
    private static bool TryGetLaunchProduct(UnifiedGame game, out string productId)
    {
        ...
        productId = match.Groups["slug"].Value;
